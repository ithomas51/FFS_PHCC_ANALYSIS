...
Run:          python scripts/integra_rate_analysis_v2_formulas.py
...
def _write_contract_tab(ws: Worksheet, df: pd.DataFrame,
    ...
    Writes XL formulas for delta / delta% columns."""
    if df.empty:
        return
    ...
    # Build column-letter map for formulas
    col_letters = {h: get_column_letter(ci) for ci, h in enumerate(cols, 1)}

    # Determine formula source columns depending on schema
    # OR Contracted: proposed = "Integra Comm", current = "Commercial $"
    # Participating: proposed = "Integra Comm", current = "Current $"
    proposed_col = "Integra Comm"
    current_col = "Commercial $" if "Commercial $" in cols else "Current $"
    cv_formulas = {}
    if proposed_col in col_letters and current_col in col_letters:
        p = col_letters[proposed_col]
        c = col_letters[current_col]
        cv_formulas["Δ Integra–Current"] = (
            f'=IF(OR({p}{{r}}="",{c}{{r}}=""),"",{p}{{r}}-{c}{{r}})')
        cv_formulas["Δ%"] = (
            f'=IF(OR({p}{{r}}="",{c}{{r}}="",{c}{{r}}=0),"",'
            f'({p}{{r}}-{c}{{r}})/{c}{{r}}*100)')
    ...
    for ri, (_, drow) in enumerate(df.iterrows(), 2):
        for ci, col_name in enumerate(cols, 1):
            ...
            # Formula columns
            if col_name in cv_formulas:
                cell.value = cv_formulas[col_name].format(r=ri)
            else:
                val = drow[col_name]
            ...
            cell.border = THIN_BORDER
            if col_name in currency_names:
                cell.number_format = CURRENCY
            elif col_name in pct_names:
...
# Formula definitions — map column name → Excel formula template.
# {src} placeholders are replaced with column letters at write time.
PAYER_FORMULAS = {
    "Δ Proposed–PHCC":  '=IF(OR({Proposed Rate}{r}="",{PHCC Current}{r}=""),"",{Proposed Rate}{r}-{PHCC Current}{r})',
    "Δ%":              '=IF(OR({Proposed Rate}{r}="",{PHCC Current}{r}="",{PHCC Current}{r}=0),"",({Proposed Rate}{r}-{PHCC Current}{r})/{PHCC Current}{r}*100)',
    "Proposed–CMS NR": '=IF(OR({Proposed Rate}{r}="",{CMS NR}{r}=""),"",{Proposed Rate}{r}-{CMS NR}{r})',
    "PHCC–CMS NR":     '=IF(OR({PHCC Current}{r}="",{CMS NR}{r}=""),"",{PHCC Current}{r}-{CMS NR}{r})',
}
...
def _write_table(ws: Worksheet, df: pd.DataFrame, start_row: int) -> int:
    ...
    # Build column-letter map for formula references
    col_letters = {h: get_column_letter(ci) for ci, h in enumerate(cols, 1)}
    ...
    for ri, (_, drow) in enumerate(df.iterrows(), start_row + 1):
        for ci, col_name in enumerate(cols, 1):
            ...
            # Formula columns → write XL formula instead of static value
            if col_name in PAYER_FORMULAS:
                tmpl = PAYER_FORMULAS[col_name]
                formula = tmpl.format(r=ri, **col_letters)
                cell.value = formula
            else:
                val = drow[col_name]
                if isinstance(val, float) and math.isnan(val):
...
def main():
    ...
    out_path = OUTPUT / "integra_rate_analysis_v2_formulas.xlsx"
    wb = Workbook()
    ...
    # Tabs 2-5: Per-payer detail  (table first, summary below)
    for payer_name in ["Commercial", "ASO", "Medicare", "Medicaid"]:
        df = payer_tables[payer_name]
        ws = wb.create_sheet(payer_name)
        end_row = _write_table(ws, df, 1)
        _write_payer_summary(ws, df, payer_name, end_row)
        _auto_width(ws)
